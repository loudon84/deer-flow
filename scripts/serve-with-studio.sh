...
if $DEV_MODE; then
    # Use -H 0.0.0.0 to allow external access for Next.js dev server
    # pnpm exec ensures the command runs with the correct node_modules context
    FRONTEND_CMD="pnpm exec next dev --turbo -H 0.0.0.0"
else
...
pkill -f "studio.api.app:app" 2>/dev/null || true
...
echo "  → Studio: Article Studio API"
...
STUDIO_PORT="${STUDIO_PORT:-8320}"
# Studio 使用独立的 uv 环境安装和运行
if [ ! -d "studio/.venv" ]; then
    echo "  Installing Studio dependencies..."
    (cd studio && uv venv && uv pip install -e .) > logs/studio-install.log 2>&1 || {
        echo "✗ Failed to install Studio dependencies. See logs/studio-install.log"
        cleanup
    }
fi
(cd studio && uv run python main.py > ../logs/studio.log 2>&1) &
./scripts/wait-for-port.sh "$STUDIO_PORT" 30 "Studio" || {
    echo "✗ Studio failed to start. Last log output:"
    tail -20 logs/studio.log
    cleanup
}
echo "✓ Studio started on localhost:$STUDIO_PORT"
