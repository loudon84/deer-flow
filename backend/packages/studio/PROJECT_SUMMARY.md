...
| 文档类型 | 文件路径 | 状态 | 说明 |
|---------|---------|------|------|
| 业务规格 | `.codeartsdoer/specs/studio/spec.md` | ✅ 完成 | 包含完整的业务需求、DFX约束、核心能力定义 |
| 技术设计 | `.codeartsdoer/specs/studio/design.md` | ✅ 完成 | 包含架构设计、接口设计、数据模型设计 |
| 任务清单 | `.codeartsdoer/specs/studio/tasks.md` | ✅ 完成 | 分9个阶段，包含所有实施任务 |
| 使用文档 | `backend/studio/README.md` | ✅ 完成 | 完整的安装、配置、使用说明 |
| 完整性报告 | `backend/studio/COMPLETENESS_REPORT.md` | ✅ 完成 | 文件完整性检查报告 |
...
### ✅ 代码边界约束
- `studio.*` 不 import `deerflow.agents.*`
- `studio.*` 不调用 DeerFlow 原 `create_chat_model()`
...
export ARTICLE_STUDIO_MONGODB_DB=studio
...
### 3. 初始化数据库
```bash
python -m studio.scripts.init_mongo_indexes
```
